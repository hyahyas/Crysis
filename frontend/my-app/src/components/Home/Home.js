...
import { faSun, faMoon, faUser, faEnvelope, faBullhorn, faTicketAlt, faCloudMoon } from "@fortawesome/free-solid-svg-icons";
...
const Home = () => {
    ...
    return (
        <div className={`flex flex-col min-h-screen ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
      
          <Disclosure as="nav" className={`bg-${darkMode ? 'gray-900' : 'white-800'} text-white p-4`}>
            {({ open }) => (
              <>
          ...
          <div className="flex-grow p-4">
            ...
            {/* Teams Admin */}
            <div className="mb-8">
              <h5 className={`text-lg font-bold ${darkMode ? 'text-black' : 'text-black'}`}>Your Servers (Admin)</h5>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {teamsAdmin.map((team) => (
                  <div key={team.id} className={`bg-white p-4 rounded-md shadow-md ${darkMode ? 'dark:bg-gray-300' : ''} h-full`}>
                    ...
                    <div className="mt-4 flex justify-between">
                      <button onClick={handleChatPage} className={`bg-indigo-500 px-3 py-1.5 text-white rounded-md ${darkMode ? 'dark:bg-gray-600' : ''}`}>
                        <FontAwesomeIcon icon={faEnvelope} />
                      </button>
                      <button onClick={handleAnnouncementsPage} className={`bg-indigo-500 px-3 py-1.5 text-white rounded-md ${darkMode ? 'dark:bg-gray-600' : ''}`}>
